...
'''
def set_TermCode(temp):
    
    Sets the TermCode to variable given from jQuery
    
    TermCode = temp
'''
def get_classes(TermCode, Search):
    '''
    Returns a list of valid classes within the parameters of TermCode and search criteria.
    Each item in the list is a dictionary containing:
        ('TermCode', 'ClassNumber', 'CrseId')
    '''
    return get_data('/Curriculum/SOC/v1/Terms/' + TermCode + '/Classes/Search/' + Search)['searchSOCClassesResponse']['SearchResult']

def get_schools(TermCode):
    '''
    Returns a list of valid schools within the parameters of TermCode.
    Each item in the list is a dictionary containing:
...
def get_subjects(TermCode, SchoolCode):
    '''
    Returns a list of valid subjects within the parameters of TermCode and SchoolCode.
    Each item in the list is a dictionary containing:
        ('SubjectCode', 'SubjectDescr', 'SubjectShortDescr')
    '''
    return get_data('/Curriculum/SOC/v1/Terms/' + TermCode + '/Schools/' + SchoolCode + '/Subjects')['getSOCSubjectsResponse']['Subject']

def get_catalogNbrs(TermCode, SchoolCode, SubjectCode):
    '''
    Returns a list of valid course catalogue numbers within the parameters of TermCode, SchoolCode, and SubjectCode.
    Each item in the list is a dictionary containing:
        ('CatalogNumber', 'CourseDescr')
    '''
...
def get_sectionNbrs(TermCode, SchoolCode, SubjectCode, CatalogNumber):
    '''
    Returns a list of valid section numbers within the parameters of TermCode, SchoolCode,SubjectCode and CatalogNumbers
    '''
